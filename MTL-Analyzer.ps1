                    ...
                    if (!("$Token" -eq "access_token"))
                    {
                        ...
                        if (Test-Path "$OUTPUT_FOLDER\MessageTraceLogs\CSV\Hunt.csv")
                        {
                            if([int](& $xsv count "$OUTPUT_FOLDER\MessageTraceLogs\CSV\Hunt.csv") -gt 0)
                            {
                                ...
                                Import-Csv -Path "$OUTPUT_FOLDER\MessageTraceLogs\CSV\Hunt.csv" -Delimiter "," | Select-Object ASN,OrgName | Where-Object {$_.ASN -ne '' } | Where-Object { ($_.PSObject.Properties | ForEach-Object {$_.Value}) -ne $null} | Group-Object ASN,OrgName | Select-Object Count,@{Name='ASN'; Expression={ $_.Values[0] }},@{Name='OrgName'; Expression={ $_.Values[1] }},@{Name='PercentUse'; Expression={"{0:p2}" -f ($_.Count / $Total)}} | Sort-Object Count -Descending | Export-Csv -Path "$OUTPUT_FOLDER\MessageTraceLogs\Stats\CSV\ASN.csv" -NoTypeInformation -Encoding UTF8
                            }
                        }
                        ...
                        if (Test-Path "$OUTPUT_FOLDER\MessageTraceLogs\CSV\Hunt.csv")
                        {
                            if([int](& $xsv count "$OUTPUT_FOLDER\MessageTraceLogs\CSV\Hunt.csv") -gt 0)
                            {
                                ...
                                Import-Csv -Path "$OUTPUT_FOLDER\MessageTraceLogs\CSV\Hunt.csv" -Delimiter "," -Encoding UTF8 | Select-Object FromIP,Country,"Country Name",ASN,OrgName | Where-Object {$_.FromIP -ne '' } | Where-Object {$_."Country Name" -ne '' } | Where-Object { ($_.PSObject.Properties | ForEach-Object {$_.Value}) -ne $null} | Group-Object FromIP,Country,"Country Name",ASN,OrgName | Select-Object Count,@{Name='FromIP'; Expression={ $_.Values[0] }},@{Name='Country'; Expression={ $_.Values[1] }},@{Name='Country Name'; Expression={ $_.Values[2] }},@{Name='ASN'; Expression={ $_.Values[3] }},@{Name='OrgName'; Expression={ $_.Values[4] }},@{Name='PercentUse'; Expression={"{0:p2}" -f ($_.Count / $Total)}} | Sort-Object Count -Descending | Export-Csv -Path "$OUTPUT_FOLDER\MessageTraceLogs\Stats\CSV\FromIP.csv" -NoTypeInformation -Encoding UTF8
                            }
                        }
