# ADSignInLogsGraph-Analyzer v0.2
#
...
<#
.SYNOPSIS
  ADSignInLogsGraph-Analyzer v0.2 - Automated Processing of Microsoft Entra ID Sign-In Logs for DFIR
...
.DESCRIPTION
  ...
  https://github.com/invictus-ir/Microsoft-Extractor-Suite (Microsoft-Extractor-Suite v1.3.5)
...
$Host.UI.RawUI.WindowTitle = "ADSignInLogsGraph-Analyzer v0.2 - Automated Processing of Microsoft Entra ID Sign-In Logs for DFIR"
...
# Header
Write-Output "ADSignInLogsGraph-Analyzer v0.2 - Automated Processing of Microsoft Entra ID Sign-In Logs for DFIR"
...
{
    Write-Host "[Error] No IPinfo CLI Access Token provided. Please add your personal access token in Line 119." -ForegroundColor Red
    Write-Host ""
    Stop-Transcript
    $Host.UI.RawUI.WindowTitle = "$DefaultWindowsTitle"
    Exit
...
$Timestamp = ($Data | Select-Object CreatedDateTime -First 1).CreatedDateTime
...
# Time Frame
$StartDate = ($Data | Select-Object @{Name="CreatedDateTime";Expression={([DateTime]::ParseExact($_.CreatedDateTime, "$TimestampFormat", [cultureinfo]::InvariantCulture).ToString("yyyy-MM-dd HH:mm:ss"))}} | Sort-Object { $_.CreatedDateTime -as [datetime] } -Descending | Select-Object -Last 1).CreatedDateTime
$EndDate = ($Data | Select-Object @{Name="CreatedDateTime";Expression={([DateTime]::ParseExact($_.CreatedDateTime, "$TimestampFormat", [cultureinfo]::InvariantCulture).ToString("yyyy-MM-dd HH:mm:ss"))}} | Sort-Object { $_.CreatedDateTime -as [datetime] } -Descending | Select-Object -First 1).CreatedDateTime
...
ForEach($Record in $Data)
{
    ...
    "CreatedDateTime"                = ($Record | Select-Object @{Name="CreatedDateTime";Expression={([DateTime]::ParseExact($_.CreatedDateTime, "$TimestampFormat", [cultureinfo]::InvariantCulture).ToString("yyyy-MM-dd HH:mm:ss"))}}).CreatedDateTime
...
if (Get-Module -ListAvailable -Name ImportExcel)
{
    if (Test-Path "$OUTPUT_FOLDER\SignInLogsGraph\CSV\Untouched.csv")
    {
        if([int](& $xsv count -d "," "$OUTPUT_FOLDER\SignInLogsGraph\CSV\Untouched.csv") -gt 0)
        {
            ...
            Set-Format -Address $WorkSheet.Cells["A1:BP1"] -BackgroundColor $BackgroundColor -FontColor Black
                        ...
                        if (Get-Module -ListAvailable -Name ImportExcel)
                        {
                            if (Test-Path "$OUTPUT_FOLDER\IpAddress\IPinfo\IPinfo-Custom.csv")
                            {
                                if([int](& $xsv count "$OUTPUT_FOLDER\IpAddress\IPinfo\IPinfo-Custom.csv") -gt 0)
                                {
                                    ...
                                    # BackgroundColor and FontColor for specific cells of TopRow
                                    $BackgroundColor = [System.Drawing.Color]::FromArgb(255,220,0)
                                    Set-Format -Address $WorkSheet.Cells["A1:K1"] -BackgroundColor $BackgroundColor -FontColor Black
                        ...
                        if (Test-Path "$OUTPUT_FOLDER\SignInLogsGraph\CSV\Untouched.csv")
                        {
                            if([int](& $xsv count "$OUTPUT_FOLDER\SignInLogsGraph\CSV\Untouched.csv") -gt 0)
                            {
                                ...
                                ForEach($Record in $Records)
                                {
                                    ...
                                    $Ticks = (Get-Date $CreatedDateTime).Ticks

                                    # 1 Tick is 100 nanoseconds
                                    # January 1, 0001, 00:00:00 --> 0 Ticks

                                    # 1 min is 20*60
                                    # 1 h is 20*60*60
                                    # 1 day is 20*60*60*24
                                    # 1 year sind20*60*60*24*365
                                    # 100 Ticks are 5 seconds
                                    # 60 Ticks are 3 seconds

                                    # Import-Csv "C:\Users\evild3ad\Desktop\ADSignInLogsGraph-Analyzer\SignInLogsGraph\CSV\Hunt.csv" -Delimiter "," | Select-Object CreatedDateTime,Ticks,IpAddress,ASN
                        ...
                        if (Get-Module -ListAvailable -Name ImportExcel)
                        {
                            if (Test-Path "$OUTPUT_FOLDER\SignInLogsGraph\CSV\Hunt.csv")
                            {
                                if([int](& $xsv count "$OUTPUT_FOLDER\SignInLogsGraph\CSV\Hunt.csv") -gt 0)
                                {
                                    ...
                                    # BackgroundColor and FontColor for specific cells of TopRow
                                    $BackgroundColor = [System.Drawing.Color]::FromArgb(255,220,0)
                                    Set-Format -Address $WorkSheet.Cells["A1:AY1"] -BackgroundColor $BackgroundColor -FontColor Black
...
if (Get-Module -ListAvailable -Name ImportExcel)
{
    if (Test-Path "$OUTPUT_FOLDER\SignInLogsGraph\Stats\CSV\ErrorCode.csv")
    {
        if([int](& $xsv count -d "," "$OUTPUT_FOLDER\SignInLogsGraph\Stats\CSV\ErrorCode.csv") -gt 0)
        {
            ...
            # BackgroundColor and FontColor for specific cells of TopRow
            $BackgroundColor = [System.Drawing.Color]::FromArgb(255,220,0)
            Set-Format -Address $WorkSheet.Cells["A1:F1"] -BackgroundColor $BackgroundColor -FontColor Black
            # HorizontalAlignment "Center" of columns A-D
            $WorkSheet.Cells["A:D"].Style.HorizontalAlignment="Center"
...
if (Get-Module -ListAvailable -Name ImportExcel)
{
    if (Test-Path "$OUTPUT_FOLDER\SignInLogsGraph\Stats\CSV\IpAddress.csv")
    {
        if([int](& $xsv count -d "," "$OUTPUT_FOLDER\SignInLogsGraph\Stats\CSV\IpAddress.csv") -gt 0)
        {
            ...
            # BackgroundColor and FontColor for specific cells of TopRow
            $BackgroundColor = [System.Drawing.Color]::FromArgb(255,220,0)
            Set-Format -Address $WorkSheet.Cells["A1:G1"] -BackgroundColor $BackgroundColor -FontColor Black
    ...
    if (Get-Module -ListAvailable -Name ImportExcel)
    {
        if (Test-Path "$OUTPUT_FOLDER\SignInLogsGraph\Brute-Force-Attack\CSV\Brute-Force-Attack-Overview.csv")
        {
            if([int](& $xsv count -d "," "$OUTPUT_FOLDER\SignInLogsGraph\Brute-Force-Attack\CSV\Brute-Force-Attack-Overview.csv") -gt 0)
            {
                ...
                # BackgroundColor and FontColor for specific cells of TopRow
                $BackgroundColor = [System.Drawing.Color]::FromArgb(255,220,0)
                Set-Format -Address $WorkSheet.Cells["A1:B1"] -BackgroundColor $BackgroundColor -FontColor Black
    ...
    if (Get-Module -ListAvailable -Name ImportExcel)
    {
        if (Test-Path "$OUTPUT_FOLDER\SignInLogsGraph\Brute-Force-Attack\CSV\Brute-Force-Attack.csv")
        {
            if([int](& $xsv count -d "," "$OUTPUT_FOLDER\SignInLogsGraph\Brute-Force-Attack\CSV\Brute-Force-Attack.csv") -gt 0)
            {
                ...
                # BackgroundColor and FontColor for specific cells of TopRow
                $BackgroundColor = [System.Drawing.Color]::FromArgb(255,220,0)
                Set-Format -Address $WorkSheet.Cells["A1:AU1"] -BackgroundColor $BackgroundColor -FontColor Black
    ...
    if (Get-Module -ListAvailable -Name ImportExcel)
    {
        if (Test-Path "$OUTPUT_FOLDER\SignInLogsGraph\AiTM\CSV\AiTM.csv")
        {
            if([int](& $xsv count "$OUTPUT_FOLDER\SignInLogsGraph\AiTM\CSV\AiTM.csv") -gt 0)
            {
                ...
                # BackgroundColor and FontColor for specific cells of TopRow
                $BackgroundColor = [System.Drawing.Color]::FromArgb(255,220,0)
                Set-Format -Address $WorkSheet.Cells["A1:AW1"] -BackgroundColor $BackgroundColor -FontColor Black
    ...
    if (Get-Module -ListAvailable -Name ImportExcel)
    {
        if (Test-Path "$OUTPUT_FOLDER\SignInLogsGraph\AiTM\CSV\AiTM_Hunt.csv")
        {
            if([int](& $xsv count "$OUTPUT_FOLDER\SignInLogsGraph\AiTM\CSV\AiTM_Hunt.csv") -gt 0)
            {
                ...
                # BackgroundColor and FontColor for specific cells of TopRow
                $BackgroundColor = [System.Drawing.Color]::FromArgb(255,220,0)
                Set-Format -Address $WorkSheet.Cells["A1:I1"] -BackgroundColor $BackgroundColor -FontColor Black
    ...
    if (Get-Module -ListAvailable -Name ImportExcel)
    {
        if (Test-Path "$OUTPUT_FOLDER\SignInLogsGraph\DeviceCode\CSV\DeviceCode-Alert2.csv")
        {
            if([int](& $xsv count "$OUTPUT_FOLDER\SignInLogsGraph\DeviceCode\CSV\DeviceCode-Alert2.csv") -gt 0)
            {
                ...
                # BackgroundColor and FontColor for specific cells of TopRow
                $BackgroundColor = [System.Drawing.Color]::FromArgb(255,220,0)
                Set-Format -Address $WorkSheet.Cells["A1:BO1"] -BackgroundColor $BackgroundColor -FontColor Black
